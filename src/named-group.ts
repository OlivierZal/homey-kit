...
// loudly instead of yielding an empty string a sweep would then account
// for as something it had read. Shared by every kernel that parses
// with named groups (route guards, floor closure, webview stamping).
export const namedGroup = (match: RegExpExecArray, name: string): string => {
  const value = match.groups?.[name]
  if (value === undefined) {
    throw new Error(
      `\`${name}\` is not a group of the pattern that matched \`${match[0]}\``,
    )
  }
  return value
}
